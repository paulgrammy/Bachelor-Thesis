...
static void gpio_event_loop(void *pvParameters)
{
    ...
    TickType_t start_time = 0;                                  // When was the button initially pressed?
    ...
    while (true)
    {
        //Pull-up => HIGH = not pressed, LOW = pressed! 
        ...
        if (!current_button_state && last_button_state)         // HIGH to LOW transition = detected press
        {
            ...
            start_time = xTaskGetTickCount();                   // Start the press timer to check if longpress
        ...
        if (button_pressed && !has_triggered_long_press)        // If button is held and hasn't triggered long press action yet
        {
            ...
            {
                has_triggered_long_press = true;                // Mark that long press action has been triggered
